#!/usr/bin/env bash
# recall PostToolUseFailure hook (v0.4.2, *braid* correlator) — write the
...
# Claude sessions from cross-pollinating. The harness passes session_id
# in the input JSON's `.session_id` field; older v0.4.1 read it from
# $CLAUDE_SESSION_ID, which the harness does not actually export — every
# fire silently no-op'd. v0.4.2 reads JSON first, env as fallback.
#
